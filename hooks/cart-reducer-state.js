...
const isValidItem = (item) => {
  return item && Number.isFinite(item.price) && Number.isInteger(item.quantity);
}
/**
 * `findOneById(items, id)` 依照某id找出項目。如果沒有找到，則返回空物件。
 */
export const findOneById = (id,size,items, pid) => {
  return items.find((item) => String(item.pid) === String(pid)&& String(item.size)===String(size)&& String(item.id)===String(id)) || {}
}
/**
...
export const updateOne = (items, updateItem) => {
  return items.map((item) => {
    if (String(item.pid) === String(updateItem.pid)&& String(item.size)===String(updateItem.size)) return updateItem
    else return item
  })
}
/**
 ...
 */
export const incrementOne = (items, id,size) => {
  return items.map((item) => {
    if (String(item.pid) === String(id)&& String(item.size)===String(size))
...
export const addOne = (items, newItem) => {
  ...
  const foundIndex = items.findIndex(
    (item) => String(item.pid) === String(newItem.pid)&& String(item.size)===String(newItem.size)
  )
 ...
 */
export const removeOne = (cartItems, pid, size,id) => {
  return cartItems.filter(item => !(item.pid === pid && item.size === size && item.id === id));
};
