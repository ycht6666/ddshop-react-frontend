...
export const CartProvider = ({
   ...
   * 給定一id值，將這商品移出陣列中
   */
  const removeItem = (pid, size,id) => {
    setCartItems(removeOne(cartItems, pid, size,id));
  };
  /**
   ...
   */
  const updateItemQty = (id,size,pid, quantity) => {
    const item = findOneById(id,size,cartItems, pid)
    // 如果沒有id，則不更新
    if (!item.pid) return
    // 更新項目
  ...
  const increment = (id,size) => {
    setCartItems(incrementOne(cartItems, id,size))
  }
  /**
